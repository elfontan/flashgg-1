...
oldargv = sys.argv[:]
sys.argv = ['-b-']
from ROOT import gStyle, gROOT, TFile, TCanvas, TLegend, kRed, kViolet, TLine
gROOT.SetBatch(True)
sys.argv = oldargv
gStyle.SetOptStat(0)
gStyle.SetOptFit(1)
...
for var_dict in variables:
    ...
    gghfile = TFile(f"Plots/{var}/gen_ggh30.root", "READ")
    genggh = gghfile.Get(f"gen{var.lower()}ggh")
    ...
    gghfileacc = TFile(f"Plots/{var}/gen_ggh30acc.root", "READ")
    gengghacc = gghfileacc.Get(f"gen{var.lower()}gghacc")
    ...
    if var == "DR":
        leg.AddEntry(genggh, f"ggH: {np.round(genggh.Integral(0, 12) / genggh.Integral() * 100, 2)}% events with dR < 0.3", "l")
        leg.AddEntry(genalp, f"ALP: {np.round(genalp.Integral(0, 12) / genalp.Integral() * 100, 2)}% events with dR < 0.3", "l")
        leg.AddEntry(gengghacc, f"ggH: {round(gengghacc.Integral() / genggh.Integral() * 100, 2)}% in Acceptance Region and", "l")
        leg.AddEntry(0, f"{np.round(gengghacc.Integral(0, 12) / gengghacc.Integral() * 100, 2)}% in Acceptance Region with dR < 0.3", "")
        leg.AddEntry(genalpacc, f"ALP: {round(genalpacc.Integral() / genalp.Integral() * 100, 2)}% in Acceptance Region and", "l")
        ...
        leg.AddEntry(gengghacc, f"ggH: {round(gengghacc.Integral() / genggh.Integral() * 100, 2)}% in Acceptance Region")
    ...
    c1.Update()
    c1.SaveAs(f"/eos/user/e/elfontan/www/Hgg_veryLowMass_AN/Interpretation/ALPsimulation/ggh_vs_alp/sig_gen_{var.lower()}_30GeV.png")
    c1.SaveAs(f"/eos/user/e/elfontan/www/Hgg_veryLowMass_AN/Interpretation/ALPsimulation/ggh_vs_alp/sig_gen_{var.lower()}_30GeV.pdf")
    ...
    c1.Update()
    c1.SetLogy()
    c1.SaveAs(f"/eos/user/e/elfontan/www/Hgg_veryLowMass_AN/Interpretation/ALPsimulation/ggh_vs_alp/sig_gen_{var.lower()}_30GeV_log.png")
    c1.SaveAs(f"/eos/user/e/elfontan/www/Hgg_veryLowMass_AN/Interpretation/ALPsimulation/ggh_vs_alp/sig_gen_{var.lower()}_30GeV_log.pdf")
